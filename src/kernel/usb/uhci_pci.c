...
int uhci_pci_init(struct pci_dev *pci)
{
    ...
    //  pci_write_dev_reg16(pci, PCI_DEV_COMMAND, 0x0005);   /* Bus master & IO
    //  space enabled */
    ...
    if ((err = uhci_init(uh)) < 0) return err;
...
{
    switch (ureg) {
    case UHCI_COMMAND: return inw(iobase);
    case UHCI_STATUS: return inw(iobase + 0x02);
    case UHCI_INT_EN: return inw(iobase + 0x04);
    case UHCI_FRAME_NUM: return inw(iobase + 0x06);
    case UHCI_FRAME_BASE_ADDR: return inl(iobase + 0x08);
    case UHCI_START_OF_FRAME_MD: return inb(iobase + 0x0C);
    default: return 0;
    }
    return 0;
}
...
{
    switch (ureg) {
    case UHCI_COMMAND: outw(iobase + 0x00, (uint16_t) data); return;
    case UHCI_STATUS: outw(iobase + 0x02, (uint16_t) data); return;
    case UHCI_INT_EN: outw(iobase + 0x04, (uint16_t) data); return;
    case UHCI_FRAME_NUM: outw(iobase + 0x06, (uint16_t) data); return;
    case UHCI_FRAME_BASE_ADDR: outl(iobase + 0x08, data); return;
    case UHCI_START_OF_FRAME_MD: outb(iobase + 0x0C, (uint8_t) data);
    default: return;
    }
}
