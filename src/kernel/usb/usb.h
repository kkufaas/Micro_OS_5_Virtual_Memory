...
#define MAX_USB_PIPES 16
    struct usb_pipe
            pipe[MAX_USB_PIPES]; /* This usb device communication pipes */
};
...
struct ATTR_PACKED usb_configuration_header {
    uint8_t  bLength;
    uint8_t  bDescriptorType;
    uint16_t wTotalLength;
};

struct ATTR_PACKED usb_generic_header {
    uint8_t bLength;
    uint8_t bDescriptorType;
};

struct ATTR_PACKED usb_dev_setup_request {
...
struct ATTR_PACKED usb_dev_descriptor {
    uint8_t  bLength;
    uint8_t  bDescriptorType;
    uint16_t bcdUSB;
...
struct ATTR_PACKED usb_dev_configuration {
    uint8_t  bLength;
    uint8_t  bDescriptorType;
    uint16_t wTotalLength;
...
struct ATTR_PACKED usb_conf_interface {
    uint8_t bLength;
    uint8_t bDescriptorType;
...
struct ATTR_PACKED usb_ifc_endpoint {
    uint8_t  bLength;
    uint8_t  bDescriptorType;
