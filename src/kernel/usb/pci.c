...
static void pci_print_device_info(struct pci_dev *dev)
{
    ...
    DEBUG("  Class: 0x%02x, Subclass: 0x%02x, ProgIF: 0x%02x", dev->class_code,
          dev->subclass_code, dev->prog_ifc);
    DEBUG("  Location:  Bus: %d, Slot: %d", dev->bus, dev->slot);
 ...
 */
static int
pci_init_device(unsigned int bus, unsigned int slot, unsigned int func)
{
    struct pci_dev *dev;
    ...
    if (err == 0) dev->op_state = DEV_STATUS_OPERATING;
    else if (err == ERR_NO_DRIVER) dev->op_state = DEV_STATUS_NO_DRIVER;
    else if (err == ERR_NO_MEM) dev->op_state = DEV_STATUS_NO_DRIVER;
    else dev->op_state = DEV_STATUS_UNKNOWN;
     ...
     */
    if ((dev->header & PCI_DEV_HEADER_MF_BIT) != 0) multiple_functions = 1;
    else multiple_functions = 0;
 ...
 * Many PCI devices can use the same interrupt line. However, we leave
 * verification of the interrupt event for the device driver.
 */
...
set_configuration_address(int bus, int slot, int func, int offset)
{
    ...
    address = (1 << 31) | (bus << 16) | (slot << 11) | (func << 8)
              | (offset & 0xfc);
...
#define pci_write_dev_reg(x, out) \
    void pci_write_dev_reg##x( \
            struct pci_dev *dev, uint32_t offset, uint##x##_t data \
    ) \
    { \
        set_configuration_address(dev->bus, dev->slot, dev->func, offset); \
