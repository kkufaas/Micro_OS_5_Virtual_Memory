...
int ehci_pci_init(struct pci_dev *pci)
{
    ...
    hccreg_base =
            (uint8_t *) (pci_read_dev_reg32(pci, EHCI_PCIREG_BASE) & 0xffffff00);
    DEBUG("   address %p", hccreg_base);
    ...
    eecp       = (hcc_params & 0x0000ff00)
           >> 16; /* EHCI extended capability pointer */
    DEBUG("   EECP %x", eecp);
    ...
    *usbcmd = 0x2;
    ...
    i = 0;
    while (*((volatile uint32_t *) usbcmd) & 0x2) i++;
