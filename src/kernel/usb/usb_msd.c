...
int usb_msd_init(struct usb_dev *udev)
{
    ...
    if (usb_msd == NULL) return ERR_NO_MEM;
    ...
    for (pipe_i = 1; pipe_i < udev->pipe_count; pipe_i++) {
        if (udev->pipe[pipe_i].attributes == USB_PIPE_DATA_BULK) {
            ...
            else usb_msd->bulk_out = &udev->pipe[pipe_i];
        }
    }
...
static int usb_msd_read_write(
        struct usb_msd_dev *umd,
        int                 dir,
        int                 cb_size,
        char               *cb_data,
        int                 length,
...
)
{
     ...
     */
    if (cb_size > CB_MAX_SIZE) return ERR_PROTO;
    ...
    if (rc != cbw_size) goto reset_dev;

    /* Read/write data from/to the mass storage device */
    if (length > 0) {
        if (dir == MSD_READ) rc = usb_read(umd->bulk_in, length, data);
        else rc = usb_write(umd->bulk_out, length, data);

        if (rc < length) goto reset_dev;
    }
 ...
 */
static int
usb_msd_read(void *driver, int cb_size, char *cb_data, int size, char *data)
{
...
static int
usb_msd_write(void *driver, int cb_size, char *cb_data, int size, char *data)
{
    struct usb_msd_dev *umd = (struct usb_msd_dev *) driver;
