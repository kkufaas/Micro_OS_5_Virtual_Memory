...
static void normal_h(struct usb_keyboard *uk, uint8_t key_code)
{
    ...
    else c = usb_keyboard[key_code].key1;
...
static void usb_kbd_ihandler(void *data)
{
    ...
    uk->shift_key = (uk->report[0] & (LEFT_SHIFT | RIGHT_SHIFT)) != 0 ? 1 : 0;
    ...
    for (i = 2; i < BOOT_REPORT_SIZE; i++) {
        ...
        if (key_pressed && key_code < 0x64) {
            ...
            if (key_code >= 0x54 && key_code <= 0x63)
                if (!uk->num_lock_key) func = usb_keyboard[key_code].func1;

            if (func != NULL) func(uk, key_code);
        }
    }

    for (i = 0; i < BOOT_REPORT_SIZE; i++) uk->last_report[i] = uk->report[i];
    c = uk->report;
    UNUSED(c);
}
