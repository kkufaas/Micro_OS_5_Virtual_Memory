...
static spinlock_t       scsi_dev_lock = SPINLOCK_INIT;

static int
scsi_read_write(int dir, int block_start, int block_count, char *data);
...
int scsi_up() { return ((scsi == NULL) ? 0 : 1); }
...
static void scsi_request_sense(void)
{
    ...
    rc = scsi->read(
            scsi->driver, sizeof(cdb10), (char *) &cdb10, sizeof(sdata),
            (char *) &sdata
    );
...
static int scsi_test_unit_ready(void)
{
    ...
    if (rc == SCSI_RC_FAILED) {
        ...
        /* Send REQUEST SENSE command. It should contain info about what went
         * wrong in previous command, and, more importantly, if we have a UNIT
         * ATTENTION condition (e.g. media change or power on reset), this
         * condition will be cleared by the device (meaning the next retry
         * might succeed).
         */
        scsi_request_sense();
        return -1;
...
static int scsi_read_capacity(void)
{
    ...
    rc = scsi->read(
            scsi->driver, sizeof(cdb10), (char *) &cdb10, cap_data_size,
            (char *) &cap_data
    );

    if (rc != SCSI_RC_GOOD) return -1;
...
scsi_read_write(int dir, int block_start, int block_count, char *data)
{
    ...
    if (dir == SCSI_READ)
        rc = scsi->read(
                scsi->driver, sizeof(cdb10), (char *) &cdb10, transfer_len,
                data
        );
    else
        rc = scsi->write(
