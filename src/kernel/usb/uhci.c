 ...
 */
static void
td_reactivate(struct usb_pipe *pipe, struct uhci_transfer_descriptor *td)
{
 ...
 */
static struct uhci_td_container *
td_build(struct usb_pipe *pipe, uint8_t pid, int size, char *data)
{
    ...
    td->token =
            (size - 1) << UHCI_TD_MAX_LEN_OFF | /* Transfer size    */
            pipe->toggle_bit << UHCI_TD_DATA_TOGGLE_OFF | /* Toggle bit */
            pipe->ep_address << UHCI_TD_END_POINT_OFF | /* Endpoint address */
            pipe->udev->address << UHCI_TD_DEV_ADDR_OFF | /* Device address */
...
static struct uhci_transfer_unit *xfer_alloc()
{
    ...
    {
        /*
         * Does this transfer unit belong to some UHCI? If so, check whether
         * it has been released in this time frame.
         * We do not reuse transfer units that has been released in this
         * time frame, since this can be still processed by UHCI.
         */
        if (xfer->uh != NULL) {
            uh        = xfer->uh;
            frame_num = uhci_pci_read(uh->iobase, UHCI_FRAME_NUM);
            if (xfer->frame_num == frame_num) continue;
        }
 ...
 */
static void
xfer_add_tdc(struct uhci_transfer_unit *xfer, struct uhci_td_container *tdc)
{
...
)
{
    ...
    if ((uhci_qh->vert_lp & UHCI_LP_TERMINATE) != 0)
         ...
         */
        next_qh          = (struct uhci_queue_head
                           *) (uhci_qh->vert_lp & UHCI_LP_ADDR_MASK);
...
static void xfer_dequeue(struct uhci_transfer_unit *xfer)
{
    ...
    if (!next_qh->is_top_queue) next_qh->prev_qh = qh->prev_qh;
    /*
    ...
    else if (prev_qh->is_top_queue) prev_qh->vert_lp = qh->horiz_lp;
    else prev_qh->horiz_lp = qh->horiz_lp;
...
void xfer_print(struct uhci_transfer_unit *xfer)
{
    ...
    if (((status >> UHCI_TD_STATUS_OFF) & 0xff) == 0) {
        ...
        if (status & UHCI_TD_ACTIVE_BIT) status_text = "ACTIVE";
        if (status & UHCI_TD_STALLED_BIT) status_text = "STALLED";
        if (status & UHCI_TD_BABBLE_BIT) status_text = "BABBLE";
        if (status & UHCI_TD_NAK_BIT) status_text = "NAK";
        if (status & UHCI_TD_TIMEOUT_BIT) status_text = "CRC/TIMEOUT";
    }
 ...
 */
static int
uhci_read_write(struct usb_pipe *pipe, int pid, int size, char *data)
{
    struct uhci_td_container  *tdc;
     ...
     */
    if (pid == UHCI_TD_PID_SETUP) pipe->toggle_bit = 0;

    if ((xfer_container = xfer_alloc()) == NULL) return ERR_NO_MEM;
    ...
    case UHCI_TD_PID_SETUP:
        ...
        break;
    default: err = ERR_PROTO; goto error_uhci_rw;
    }
     ...
     * transmitted bytes
     */
    if (err == 0) err = xfer_count_bytes(xfer_container);
...
static int
uhci_setup(struct usb_pipe *pipe, struct usb_dev_setup_request *data)
{
...
static int uhci_register_interrupt_h(struct usb_interrupt *ui)
{
    ...
    if (iq_element == NULL) return ERR_NO_MEM;
...
void uhci_interrupt(struct uhci *uh)
{
    ...
    if ((int_status & UHCI_SS_USB_INT) != 0) {
        ...
        LIST_FOR_EACH(&uh->interrupt_handler_list_head, iq_i)
        {
            ...
            pipe           = iq_i->ui->pipe;
            xfer_container = (struct uhci_transfer_unit *) pipe->xfer;
            ...
            /* Reactivate this transfer (this ensures at least 2ms freq) */
            pipe->toggle_bit = 1 - pipe->toggle_bit;
...
enum port_status_e uhci_port_status(struct usb_hub *uhub, int port)
{
    ...
    else if ((status & UHCI_PORTSC_ENABLE) == 0) rc = USB_PORT_DISABLED;
    else if ((status & UHCI_PORTSC_SUSPEND) == 0) rc = USB_PORT_ENABLED;
    else rc = USB_PORT_SUSPENDED;

    return rc;
}

void uhci_port_command(
        struct usb_hub *uhub, int port, enum uhub_port_command_e command
)
{
    struct uhci *uh = (struct uhci *) uhub->hc;

    switch (command) {
    case USB_PORT_RESET: uhci_port_set_bit(uh, port, UHCI_PORTSC_RESET); break;
...
enum usb_speed_class_e uhci_port_speed(struct usb_hub *uhub, int port)
{
    ...
    return ((uhci_pci_read_portsc(uh->iobase, port) & UHCI_PORTSC_LS_DEV) != 0)
                   ? USB_LS_DEV
                   : USB_FS_DEV;
}

/* Hub operations */
struct usb_hub_ops uhci_hub_ops = {
        .port_speed   = uhci_port_speed,
        .port_command = uhci_port_command,
        .port_status  = uhci_port_status};

/* Host controler operations */
struct usb_hc_ops uhci_hc_ops = {
        .read                 = uhci_read,
        .write                = uhci_write,
        .setup                = uhci_setup,
        .get_next_addr        = uhci_get_next_address,
        .register_interrupt_h = uhci_register_interrupt_h,
...
int uhci_init(struct uhci *uh)
{
    ...
    uh->xfer_qh_lock = (spinlock_t) SPINLOCK_INIT;
    ...
    uh->frame_base_addr = kzalloc_align(UHCI_FRAME_LIST_SIZE * 4, (1 << 12));
    ...
    frame_list_pointer = uh->frame_base_addr;
    for (i = 0; i < UHCI_FRAME_LIST_SIZE; i++)
        *frame_list_pointer++ = ((uint32_t) uh->interrupt_qh) | UHCI_LP_QH;
    ...
    uhci_pci_write(
            uh->iobase, UHCI_FRAME_BASE_ADDR, (uint32_t) uh->frame_base_addr
    );
    ...
    /* Enable UHCI */
    uhci_pci_write(
            uh->iobase, UHCI_COMMAND, 0x00C1
    ); /* 0x80 64B packets allowed at SOF
    ...
    if (root_hub == NULL) return ERR_NO_MEM;
