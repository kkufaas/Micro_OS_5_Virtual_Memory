...
)
{
    ...
    if (rc != 8) return ERR_XFER;
    /* Read/write stage if necessary */
    if (size > 0) {
        if (dir == USB_CREAD) rc = udev->hc_ops->read(pipe, size, buff);
        else rc = udev->hc_ops->write(pipe, size, buff);

        if (rc != size) return ERR_XFER;
    }
    /* Status stage (always uses toggle bit 1) */
    pipe->toggle_bit = 1;
    if (dir == USB_CREAD) rc = udev->hc_ops->write(pipe, 0, NULL);
    else rc = udev->hc_ops->read(pipe, 0, NULL);

    if (rc != 0) return ERR_XFER;
 ...
 */
static int
load_configuration(struct usb_dev *udev, int conf_index, char *conf_buff)
{
    ...
    rc   = load_descriptor(
              udev, USB_CONFIGURATION_DESCRIPTOR, conf_index, size, (char *) &udc
      );

    if (rc < 0) return ERR_XFER;

    if (udc.bDescriptorType != USB_CONFIGURATION_DESCRIPTOR) return ERR_XFER;

    rc = load_descriptor(
            udev, USB_CONFIGURATION_DESCRIPTOR, conf_index, udc.wTotalLength,
            conf_buff
    );

    if (rc == 0) return udc.wTotalLength;
 ...
 * descriptors.
 */
static char *get_descriptor_ptr(
        int descriptor_type, int index, int conf_size, char *conf_buff
)
{
    ...
    do {
        ...
        if (ugh->bDescriptorType == descriptor_type) {
            if (index == 0) /* Return the pointer to this descriptor */
                return conf_buff;
            index--;
        }
...
int usb_configure_device(struct usb_dev *udev, int address)
{
    ...
    for (conf_i = 0; conf_i < udev->configurations_num; conf_i++) {
        ...
        for (ifc_i = 0; ifc_i < udev_conf->bNumInterfaces; ifc_i++) {
            ptr = get_descriptor_ptr(
                    USB_INTERFACE_DESCRIPTOR, ifc_i, conf_size, buff_ptr
            );
            conf_ifc = (struct usb_conf_interface *) ptr;

            /* If broken configuration then check the next one */
            if (conf_ifc == NULL) break;
...
exit_scan:
    ...
    for (ep_i = 0; ep_i < conf_ifc->bNumEndpoints; ep_i++) {
        ptr = get_descriptor_ptr(
                USB_ENDPOINT_DESCRIPTOR, ep_i, conf_size, buff_ptr
        );
        ifc_ep = (struct usb_ifc_endpoint *) ptr;

        DEBUG("Looking up endpoint addr %x attr %x", ifc_ep->bEndpointAddress,
              ifc_ep->bmAttributes);
    ...
    /* Configure address  */
    rc = send_request(
            &udev->pipe[0], USB_RECIV_DEVICE, SET_ADDRESS, (uint16_t) address,
            0
    );
    ...
    rc = send_request(
            &udev->pipe[0], USB_RECIV_DEVICE, SET_CONFIGURATION,
    ...
    /* Choose interface */
    rc = send_request(
            &udev->pipe[0], USB_RECIV_INTERFACE, SET_INTERFACE,
            conf_ifc->bAlternateSetting, conf_ifc->bInterfaceNumber
    );
