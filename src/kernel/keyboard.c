...
void normal_handler(unsigned char scan)
{
    ...
    if (key_release == false) {
        ...
        case CAPS_SHIFT:
            ...
            break;
        case CONTROL: char_read.character = scan_to_ascii[scan].control; break;
        case 0: char_read.character = scan_to_ascii[scan].no_shift; break;
        default: char_read.character = 0; break;
        }
        putchar(&char_read);
    }
}
...
{
    if (key_release == false) shift_status |= RIGHT_SHIFT;
    else shift_status &= ~RIGHT_SHIFT;
    ignore(c);
}
...
{
    if (key_release == false) shift_status |= LEFT_SHIFT;
    else shift_status &= ~LEFT_SHIFT;
    ignore(c);
}
...
{
    if (key_release == false) shift_status |= CONTROL;
    else shift_status &= ~CONTROL;
    ignore(c);
}
...
{
    if (key_release == false) shift_status |= ALT;
    else shift_status &= ~ALT;
    ignore(c);
}
...
{
    if (key_release == false) {
        if (shift_status & CAPS_SHIFT) shift_status &= ~CAPS_SHIFT;
        else shift_status |= CAPS_SHIFT;
    }
    ignore(c);
}
