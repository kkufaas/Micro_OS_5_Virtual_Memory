...
typedef struct {
    ...
    int  tail; /* points to oldest message (first to be recived) in buffer */
...
/*
 * Get number of messages (count) and number of bytes
 * available in the mailbox buffer (space). Note that the
 * buffer is also used for storing the message headers, which
 * means that a message will take sizeof(mbox_t) + m->size
 * bytes in the buffer.
 */
