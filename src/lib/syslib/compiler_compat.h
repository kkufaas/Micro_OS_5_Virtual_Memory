...
/*
 * Function attr: This function is called from an interrupt handler
 *
 * The clang compiler requires that functions called from an interrupt service
 * routine (C functions with the interrupt attribute) use the attribute
 * 'no_caller_saved_registers' (which in turn requires
 * 'target("general-regs-only")'). The warning is
 * '-Winterrupt-service-routine'.
 *
 * GCC does not give this warning and indeed does not even have the
 * '-Winterrupt-service-routine' flag. Interrupt functions generated by GCC
 * (at least as of version 11.4) simply save all registers that they use, and
 * if they call a function, they save all caller-saved registers (EAX, ECX,
 * EDX).
 *
 * This suggests that the priority for clang is to save as few registers as
 * possible during interrupts. The clang warnings push the programmer to ensure
 * that the ISR and any functions it calls all save _only_ the registers they
 * use, overriding the normal calling conventions.
 *
