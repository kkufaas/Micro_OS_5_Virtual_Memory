 ...
 * deterministically in sequence, and process4 verifies that it receives them
 * as exected. This tests your mailbox implementation's integrity.
 *
 ...
 *   This blocks until it receives a 'token' as a signal from the other
 * process.
 *
...
static struct term term = PROC3_TERM_INIT;
...
#define ACTUAL_MSG_SIZE(n) ((n) + (int) sizeof(int))
...
int main(void)
{
    ...
    for (i = 0; i < 100; i++) {
        ...
        tprintf(&term, "Process 3\n%d", i);
        tprintf(&term, ANSIF_EL, ANSI_EFWD); // Clear right

        /*
         * Create message to send
         *
         * Size and content are set deterministically from 'i', so that the
         * sequence of messages is predictable.
         */
        size    = 128 + i % 128;
        m->size = size;

        c = 'a' + i % 26;
        for (j = 0; j < size; j++) m->body[j] = c;
         ...
         * Send message
         *
         * Note: we may have just finished filling the queue. We send on this
         * iteration anyway. The send call should block until there is space.
         */
        mbox_send(q, m);
    }
     ...
     *
     * We are done sending, but it is likely that process4 is still in its wait
     * state, waiting on a signal from us to continue reading from the queue.
     * Send that signal now.
     */
    mbox_send(fo, &token);
